...
const AstronomyFact = ({ dailyFact, saveFact, savedClicked, handleFactClick }) => {
  return (
    <div className='astronomy-card card'>
      ...
      <h5 className='save-feature'>
        <button data-cy='save-button' className={!savedClicked ? 'save-button' : 'already-saved'}>
          <img className='save-button-img'src='save-img.png' alt='save button' onClick={(event) => handleFactClick(dailyFact)}/>
        </button>
